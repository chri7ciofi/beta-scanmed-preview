...
- **Camera scanning is restricted to `data_matrix` only** (`ensureBarcodeDetector()` requests `formats: ['data_matrix']`) — pharma DataMatrix reliability + lower CPU, no partial reads from other formats. EAN-13 is no longer camera-scannable; it remains reachable only via manual AIC entry (`extractAIC()`). `getUserMedia` requests Full HD (`ideal` 1920×1080) + continuous `focusMode` for the sub-millimetre modules.
- `handleScan()` routes by `code.format`: `data_matrix` → `parseGS1()` (AIC + prefill expiry/lotto); the non-`data_matrix` branch falls back to `extractAIC()` (now reachable only for non-GS1 DataMatrix payloads / manual entry).
